...
async def publish_draft_to_linkedin(draft_id: str, user=Depends(get_current_user)):
    ...
    if not d:
        raise HTTPException(404, "Brouillon introuvable")
    udoc = await db.users.find_one({"id": user["id"]}, {"linkedin": 1, "_id": 0})
    li = (udoc or {}).get("linkedin") or {}
    if not li.get("access_token"):
        raise HTTPException(401, "LinkedIn non connecté. Allez sur Brouillons et cliquez sur 'Connecter LinkedIn'.")

    access_token = dec(li["access_token"])
    member_urn = li.get("member_urn")
    if not member_urn:
        raise HTTPException(400, "LinkedIn member URN manquant. Reconnectez-vous à LinkedIn.")

    # Generate LinkedIn post text via Claude
    post_text = await _generate_linkedin_post_text(d)

    # Get public URL of the article (from previous GitHub publish if any)
